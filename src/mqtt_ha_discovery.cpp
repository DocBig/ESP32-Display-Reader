...
static String autoDeviceClassForRoi(const Roi &roi) {
  ...
  static const char *heatKeys[] = {"heat", "heating", "burner"};
  static const char *waterKeys[] = {"water", "wasser"};
  if (roi.type == "symbol") {
  ...
  if (containsAny(s, waterKeys, sizeof(waterKeys) / sizeof(waterKeys[0]))) return "water";
...
static String autoStateClassForRoi(const Roi &roi) {
  ...
  static const char *waterKeys[] = {"water", "wasser"};
  if (containsAny(s, waterKeys, sizeof(waterKeys) / sizeof(waterKeys[0]))) {
    return "total_increasing";
  }
