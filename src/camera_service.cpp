...
static bool fillCameraPins(camera_config_t &c, const String &model) {
  ...
  if (model == "atoms3r_m12") {
    // M5Stack AtomS3R M12 (OV3660), Pins laut offiziellem M5Stack-Beispiel
    // (m5stack/M5AtomS3, examples/Basics/camera/camera_pins.h). UNGETESTET.
    c.pin_pwdn = -1;
    c.pin_reset = -1;
    c.pin_xclk = 21;
    c.pin_sccb_sda = 12;
...
bool cameraStart(const AppConfig &cfg) {
  ...
  // AtomS3R-Boards besitzen eine 5V->3.3V Power-Management-Schaltung fuer das
  // Kameramodul, die ueber GPIO18 aktiviert wird (unabhaengig vom PWDN-Pin
  // im camera_config_t, der hier -1/ungenutzt ist). Ohne diesen Schritt
  // bleibt die Kamera unversorgt und esp_camera_init() schlaegt fehl oder
  // haengt. UNGETESTET, Wert laut M5Stack-Referenzcode.
  if (cfg.camera.model == "atoms3r_m12") {
    pinMode(18, OUTPUT);
    digitalWrite(18, LOW);
    delay(500); // Power-Stabilisierung
  }
